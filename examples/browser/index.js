...
const openDatabase = () => {
  ...
  const ipfs = new IPFS({
    repo: '/orbitdb/examples/browser/' + time + '/ipfs',
    EXPERIMENTAL: {
      pubsub: true,
    },
  })
  ...
  ipfs.on('ready', () => {
    ...
    ipfs.id()
      .then((id) => {
        ...
        const db = orbitdb.kvstore(dbname, { maxHistory: 5, path: '/orbitdb/examples/browser/keyvalue-' + time })
        const log = orbitdb.eventlog(dbname + ".log", { maxHistory: 5, path: '/orbitdb/examples/browser/events-' + time })
        const counter = orbitdb.counter(dbname + ".count", { maxHistory: 5, path: '/orbitdb/examples/browser/counter-' + time })
        ...
        const getData = () => {
          ...
          ipfs.swarm.peers()
            ...
            .then((peers) => {
              const output = `
                <b>You are:</b> ${peerId} ${creature}<br>
